...
- Disconnect your machine from the internet (for example, turn off your
Wi-Fi).
-
Submit a workflow:
...
-
Check the Temporal UI (`http://localhost:8233`). You will see the LLM
...
-
The next automated retry will successfully reach the Gemini API, and your
...
- To give yourself time to kill the worker, open `durable_agent_worker.py` and
...
-
Submit a query that triggers several tools:
...
-
Kill the worker process any time before completion (`Ctrl-C` in the worker
