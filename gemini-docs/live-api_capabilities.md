...
[Gemini 3.1 Flash Live Preview](/gemini-api/docs/models/gemini-3.1-flash-live-preview) and the [Gemini 2.5 Flash Live Preview](/gemini-api/docs/models/gemini-2.5-flash-native-audio-preview-12-2025) models:
 ...
 | Not supported
 | Supported. When enabled, the model can proactively decide not to respond if the input content is not relevant. Set `proactive_audio` to `true` in the `proactivity` config (requires `v1alpha`).
 |


 | **[Affective dialogue](#affective-dialog)**
 | Not supported
 | Supported. The model adapts its response style to match the expression and tone of the input. Set `enable_affective_dialog` to `true` in session config (requires `v1alpha`).
 |
