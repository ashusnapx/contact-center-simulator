...
with explanations for their causes and troubleshooting steps:
 ...
 | To use the Gemini API, you will need to setup a paid plan using [Google AI Studio](https://aistudio.google.com/apikey).
 |
 ...
 are trying to use a tuned model without going through [proper authentication](/gemini-api/docs/model-tuning).
 ...
 | Check if all [parameters in your request are valid](/gemini-api/docs/troubleshooting#check-api) for your API version.
 |
 ...
 | RESOURCE_EXHAUSTED
 | You've exceeded one of the API's rate limits (RPM, TPM, RPD, spend, etc.).
 | You are sending too many requests, using too many tokens, or exceeding spend-based limits for your account's billing history and tier.
 | Verify that you're within the model's [rate limits](/gemini-api/docs/rate-limits). Wait and retry after a short period. Reduce the rate or size of your requests. [Request a rate limit increase](/gemini-api/docs/rate-limits#request-rate-limit-increase) if needed.
 |
 ...
 | The client closed the connection before the API could finish responding.
 | Check if your client or network infrastructure is prematurely closing the connection (e.g., due to a client-side timeout).
 |
 ...
 | Check the [Gemini API status page](https://aistudio.google.com/status) for any ongoing incidents. Reduce your input context or temporarily switch to another model (e.g. from Gemini 2.5 Pro to Gemini 2.5 Flash) and see if it works. Or wait a bit and retry your request. If the issue persists after retrying, please report it using the Send feedback** button in Google AI Studio.
 |
 ...
 | Check the [Gemini API status page](https://aistudio.google.com/status) for any ongoing incidents. Temporarily switch to another model (e.g. from Gemini 2.5 Pro to Gemini 2.5 Flash) and see if it works. Or wait a bit and retry your request. If the issue persists after retrying, please report it using the **Send feedback** button in Google AI Studio.
 |
...
Verify that your model parameters are within the following values:
 ...
 |
 Use the [models page](/gemini-api/docs/models/gemini)
