...
- [**Send a request to the model**](#send-request)
...
- [**Receive the model response**](#model-response)
...
- [**Execute the received action**](#execute-actions)
...
- [**Capture the new environment state**](#capture-state)
...
| **double_click**
| Double clicks at the coordinate.
| `y`: int (0-999)
`x`: int (0-999)
`intent`: str
|
...
| **triple_click**
| Triple clicks at the coordinate.
| `y`: int (0-999)
`x`: int (0-999)
`intent`: str
|
...
| **middle_click**
| Middle clicks at the coordinate.
| `y`: int (0-999)
`x`: int (0-999)
`intent`: str
|
...
| **right_click**
| Right clicks at the coordinate.
| `y`: int (0-999)
`x`: int (0-999)
`intent`: str
|
...
| **mouse_down**
| Presses and holds the mouse button at the coordinate.
| `y`: int (0-999)
`x`: int (0-999)
`intent`: str
|
...
| **mouse_up**
| Releases the mouse button at the coordinate.
| `y`: int (0-999)
`x`: int (0-999)
`intent`: str
|
...
| **drag_and_drop**
| Drags an item from the start coordinate to the end coordinate.
| `start_y`: int (0-999)
`start_x`: int (0-999)
`end_y`: int (0-999)
`end_x`: int (0-999)
...
| **scroll**
| Scrolls up, down, left, or right at a coordinate by a pixel distance.
| `y`: int (0-999)
`x`: int (0-999)
...
| **list_apps**
| Lists available applications on the device, returning their names and package names.
| `intent`: str
|
...
| **go_back**
| Navigates back to the previous screen or webpage.
| `intent`: str
|
...
| **long_press**
| Performs a long press at a coordinate on the screen.
| `y`: int (0-999)
`x`: int (0-999)
...
| **search**
| Navigates to default search engine.
| None
...
| **hover_at**
| Hovers mouse at a specific coordinate.
| `y`: int (0-999), `x`: int (0-999)
| `{"name": "hover_at", "arguments": {"y": 150, "x": 250}}`
|

| **type_text_at**
| Types text at a coordinate.
...
| **scroll_at**
| Scrolls at coordinate (x,y).
| `y`: int, `x`: int, `direction`: str, `magnitude`: int (Optional, default 800)
| `{"name": "scroll_at", "arguments": {"y": 500, "x": 500, "direction": "down"}}`
|

| **drag_and_drop**
| Drags between two coordinates.
| `y`: int, `x`: int, `destination_y`: int, `destination_x`: int
...
-
**Content guardrails:** Use guardrails and content safety APIs to evaluate
...
- [**Gemini 3.5 Flash**](/gemini-api/docs/models/gemini-3.5-flash) (`gemini-3.5-flash`): The recommended model for
