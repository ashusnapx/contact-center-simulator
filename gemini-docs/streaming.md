...
- `interaction.created`: The interaction is created, includes metadata (ID, model, status).
- A series of **steps**, each consisting of:
...
- An `interaction.completed` event with final `usage` statistics.
...
| Chain-of-thought reasoning. `summary` is only present when `thinking_summaries` is enabled.
|

| `function_call`
| `arguments_delta`
| A request for the client to execute a function. Sets interaction status to `requires_action`.
|

| Server-side tools
| Varies by tool
| Tools executed by the API (e.g., `google_search_call`, `google_search_result`, `code_execution_call`, `code_execution_result`).
|
...
- **Turn 1 (Function Request):** Call `interactions.create` with `stream: true`
...
`requires_action`.
- **Turn 2 (Sending Result):** Call `interactions.create` again, passing the
