...
Tool
| User visible tool call args (if any)
| User visible tool response (if any)
|
...
| Server-side
| Supported (built in, uses `code_execution` and `code_execution_result` steps)
|

| [Computer Use](/gemini-api/docs/computer-use)
| Client-side
| Supported (built in, uses `function_call` and `function_response` steps)
|

| [Custom functions](/gemini-api/docs/function-calling)
| Client-side
| Supported (built in, uses `function_call` and `function_response` steps)
|
